...
#+END_SRC
**** Complie
     I am use [[https://github.com/bard/emacs-run-command][run-command]], for run command, for latex I am have some commands (pdflatex (TEX -> PDF), pandoc (TEX -> ...)):
  ...
  see https://github.com/bard/emacs-run-command#examples."
      (-when-let (file-path (buffer-file-name))
          (when (-contains-p TeX-modes major-mode)
              (list
               (list :display "Convert to PDF, with `pdflatex`, ignore errors"
...
*** Elisp
    Emacs has cool built-in support of Elisp, but this is not include =elisp-format=, so
I am use [[https://www.emacswiki.org/emacs/ElispFormat][ElispFormat]]:
...
*** Pandoc
    I am use [[https://pandoc.org][pandoc]], for this I am have var =pandoc-output-formats=, which is list of pandoc's formats valid to pandoc output (see [[https://pandoc.org][this]]):
...
This function take pandoc code and file name with format one, return this file in format with pandoc code. So I have variable =pandoc-codes-and-extensions=:
...
This is recipe for [[https://github.com/bard/emacs-run-command][run-command]]:
