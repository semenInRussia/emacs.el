...
#+BEGIN_SRC emacs-lisp :tangle no
  ...
  (defcustom my-xah-fly-ignored-modes
    '(calc-mode)
    "List of the major modes in which `xah-fly-keys` shouldn't work."
    :type '(repeat symbol))

  (use-package xah-fly-keys
      :demand nil
      :config
      (add-to-list 'calc-mode-hook
                   'my-disable-xah-fly-keys-locally))

  (defun my-disable-xah-fly-keys-locally ()
    "Disable `xah-fly-keys' only for the current buffer."
    (interactive)
    (print "JUST AWORKDJ!")
    (setq-local xah-fly-command-map (make-sparse-keymap))
