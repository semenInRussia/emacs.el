;;; my-mipt.el --- My configuration for `mipt'
...
(defgroup my-mipt nil "Group for my `mipt' management." :group 'tool)
...
(defun my-mipt-all-tasks ()
  ...
  (-keep 'my-mipt-task-parse (f-files my-mipt-dir)))
...
Special variable is `my-mipt-found-task'"
  (let* ((class (my-mipt-choose-one-of-task-classes tasks)))
    ...
    (--filter (= (my-mipt-task-class it) class) tasks)))
...
(defun my-mipt-task-browse-course-url (&optional task)
  ...
  (browse-url (my-mipt-task-course-url task)))
...
(defun my-mipt-course-url (course-name)
  ...
  (->>
   course-name
   (my-mipt-course-id)
   (s-prepend "https://zftsh.online/course/")))
...
not saved, then save with ID readed from the minibuffer"
  (unless (my-mipt-course-id course-name)
    (let ((prompt (format "ID of the course %s: " course-name)))
      (my-mipt-course-save-id course-name (read-string prompt)))))
...
(defun my-mipt-task-save-course-id (task id)
  ...
  (my-mipt-course-save-id (my-mipt-task-course-name task) id))
