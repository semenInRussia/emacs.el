...
(defvar hl-sexp-overlay nil "The current overlay.")
...
(define-globalized-minor-mode global-highlight-sexp-mode
  highlight-sexp-mode
  (lambda () (highlight-sexp-mode t)))

(defun hl-sexp-delete-overlay ()
  (if hl-sexp-overlay (delete-overlay hl-sexp-overlay))
...
(defun hl-sexp-highlight ()
  (let ((text-property (get-text-property (point) 'face)))
    ...
    (cond
     ((or
       (eq text-property 'font-lock-string-face)
          ...
          (let* ((sppss (syntax-ppss))
            ...
            (cond
             ((and start
              ...
              (ignore-errors (setq end (scan-sexps start 1)))
              (cond
               (end
                (move-overlay hl-sexp-overlay (1+ start) (1- end)))
               (t (move-overlay hl-sexp-overlay (1+ start) (point)))))
...
(defun hl-sexp-create-overlay ()
  (let ((attribute (face-attr-construct 'hl-sexp-face)))
    (if hl-sexp-foreground-color
        (setq attribute
              (plist-put attribute :foreground hl-sexp-foreground-color)))
    (if hl-sexp-background-color
        (setq attribute
              (plist-put attribute :background hl-sexp-background-color)))
...
(defun hl-sexp-handle-clone-indirect-buffer ()
  ...
  (set
   (make-local-variable 'hl-sexp-overlay)
   (copy-overlay hl-sexp-overlay))
