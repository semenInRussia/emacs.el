...
(defgroup hare nil "HaRe options.")

(defcustom hare-search-paths
  (cons (expand-file-name ".") nil )
...
(defun hare-init-menu()
  ...
  (define-key haskell-mode-map [menu-bar mymenu dm]
    '("Demote Definition"    . hare-refactor-demote))
  (define-key haskell-mode-map [menu-bar mymenu dd]
    '("Duplicate Definition" . hare-refactor-dupdef))
  (define-key haskell-mode-map [menu-bar mymenu ic]
    '("Convert if to case"   . hare-refactor-iftocase))
  (define-key haskell-mode-map [menu-bar mymenu lo]
    '("Lift one level"       . hare-refactor-lift-one))
  (define-key haskell-mode-map [menu-bar mymenu lt]
    '("Lift to top level"    . hare-refactor-lifttotop))
  (define-key haskell-mode-map [menu-bar mymenu r ]
    '("Rename"               . hare-refactor-rename))
  (define-key haskell-mode-map [menu-bar mymenu r ]
    '("Round trip"           . hare-refactor-roundtrip))
  (define-key haskell-mode-map [menu-bar mymenu sh ]
    '("Show"                . hare-refactor-show)))
...
(defvar hare_ext
  (expand-file-name
   (concat (getenv "HOME") "/.hare/elisp/hare_ext.el")))
        ...
        (if (y-or-n-p "Do you want to preview changes made to other files?")
            (progn
              ...
              (if (get-file-buffer-1 file-to-diff)
                  nil
                (setq unopened-files
                      (cons file-to-diff unopened-files)))
              (hare-ediff file-to-diff
                          (concat
                           (file-name-sans-extension file-to-diff)
                           ".refactored"
                           (file-name-extension file-to-diff t))))
          (progn (setq modified-files nil) (commit-or-abort))))
    nil))
...
(defun commit() "commit the refactoring result."
...
(defun ghc-read-lisp (func)
  (with-temp-buffer
    (funcall func)
    (goto-char (point-min))
    (condition-case nil (read (current-buffer)) (error ()))))
...
(defun do-commit()
  ...
  (let ((files (list)))
    ...
    (dolist (uf unopened-files) (kill-buffer (get-file-buffer-1 uf)))
    (setq unopened-files nil)
    (setq refactor-mode nil)
...
(defun delete-swp-file-and-buffers (files)
  ...
  (let ((suf (current-time-suffix)))
	...
	(let ((swp-buff (get-file-buffer-1 swp-file-name)))
	  (if swp-buff (kill-buffer swp-buff) nil))
...
(defun start-hare-app() (interactive) (hare-menu-init))
...
(defun haskell-refactorer-off()
  (interactive)
  (hare-menu-remove)
  (if (not (get-buffer "*HaRe-Shell*")) t
...
(defun haskell-refactorer-on()
  ...
  ;; (check-erl-cookie)
  (if (get-buffer "*HaRe-Shell*") (haskell-refactorer-off-1) t)
  ...
  ;; (sleep-for 1.0)
  (save-window-excursion (hare-shell))
  (sleep-for 1.0)
  ;; (erl-spawn
...
(if (file-exists-p hare_ext) (load  hare_ext) nil)
...
(defun erlang-menu-uninstall (name items keymap &optional popup)
  ...
  (cond
   (erlang-xemacs-p (delete-menu-item name))
   ((>= erlang-emacs-major-version 19)
    (define-key keymap (vector 'menu-bar (intern name)) 'undefined))
   (t nil)))
...
(defun preview-commit-cancel-1 (current-file-name modified)
  "preview, commit or cancel the refactoring result"
  (let ((answer
         (read-char-spec-1 "Do you want to preview(p)/commit(c)/cancel(n) the changes to be performed?(p/c/n):"
...
(defun buffers-saved()
  (let (changed)
    ...
    (if changed
        (if (y-or-n-p
             (format "There are modified buffers: %s, which HaRe needs to save before refactoring, continue?" changed))
            (progn (save-some-buffers t) t)
          nil)
      t)))
...
(defun buffers-changed-warning()
  (let (changed)
    ...
    (if changed
        (if (y-or-n-p
             (format "Undo a refactoring could also undo the editings done after the refactoring, undo anyway?"))
            t
          nil)
      t)))
    ...
    (if (buffers-saved)
        (hare-refactor-command current-file-name start-line-no start-col-no
                               `("iftocase" ,current-file-name
                                 ,(number-to-string start-line-no)
                                 ,(number-to-string start-col-no)
                                 ,(number-to-string end-line-no)
                                 ,(number-to-string end-col-no))
    ...
    (if (buffers-saved)
        (hare-refactor-command current-file-name line-no column-no
                               `("dupdef" ,current-file-name ,name
                                 ,(number-to-string line-no)
                                 ,(number-to-string column-no))
                               hare-search-paths 'emacs tab-width)
      (message "Refactoring aborted."))))
  ...
  (let (composite-refac-p name msg result)
    ...
    (let ((res
           (ghc-read-lisp
            (lambda ()
              (message "Running...")
              (apply 'call-process ghc-hare-command nil t nil params)
              (message "Running...done")))))
      (with-current-buffer (get-buffer-create "*HaRe*")
        (insert (format "%s\n" (prin1-to-string res))))
      (message "Res=%s" res)
    ...
    (if composite-refac-p
        (cond
         ...
         ((equal (elt result 0) 'ok)
          ...
          (setq renamed
                (if (> (length result) 3) (elt result 2) nil))
          (setq warning
                (if (> (length result) 3)
                    (elt result 3)
                  (if (> (length result) 2) (elt result 2) nil)))
          (if warning (setq has-warning warning) nil)
          ;;(update-buffers modified)
          (revert-all-buffers)
          (apply-refac-cmds current-file-name (list 'ok modified)))
         ((and
           (sequencep (elt result 0))
           (equal (elt (elt result 0) 0) 'ok))
          ...
          (revert-all-buffers)
          (apply-refac-cmds current-file-name
                            (list (list 'ok modified) name_change)))
         (t
          (revert-all-buffers)
      ...
      (cond
       ((equal (elt result 0) 'ok)
        (setq modified
              (if (> (length result) 1) (elt result 1) nil))
        (setq renamed
              (if (> (length result) 3) (elt result 2) nil))
        (setq warning
              (if (> (length result) 3)
...
(defvar highlight-region-overlay
  ...
  (cond
   (erlang-xemacs-p (make-extent 1 1))
   (t (make-overlay 1 1)))
...
(defun highlight-region(line1 col1 line2 col2 buffer)
  ...
  (org-move-overlay highlight-region-overlay
                    (get-position line1 col1)
                    (get-position line2 (+ 1 col2))
                    buffer)
  (goto-line line2)
  (goto-column col2))
...
(defun highlight-instances-with-same-face(filename regions)
  ...
  (let (buffer (find-file filename))
    (with-current-buffer buffer
      (dolist (r regions) (highlight-use-instance r buffer)))))
